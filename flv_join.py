...
def write_meta_data(stream, meta_type, meta_data):
    assert isinstance(meta_type, basestring)
    write_amf(meta_type)
    write_amf(meta_data)
...
def concat_flvs(ui, flvs, output=None):
    #ui.addMsg('here in flvs join 0')
    assert flvs, 'no flv file found'
    import os.path
    #ui.addMsg('here in flvs join 1')
    if not output:
        output = guess_output(flvs)
    elif os.path.isdir(output):
        output = os.path.join(output, guess_output(flvs))
    #ui.addMsg('here in flvs join 2')
    # try:
    #     xxxxxx = 'Joining %s into %s' % (', '.join(flvs), output)
    #     print 'Joining %s into %s' % (', '.join(flvs), output)
    #     ui.addMsg('here in flvs join 3')
    #     ui.addMsg(xxxxxx)
